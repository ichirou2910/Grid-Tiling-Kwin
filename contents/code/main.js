...
var Algorithm = {
  toBool: function (
    value // QVariant can be compared with == but not === as it is a type of itself
  ) {
...
var Parameters = {
  prefix: "Grid-Tiling: ",
  grids: Algorithm.createGrids(
    readConfig("rows", "2, 2, 2, 2, 2, 2, 2, 2, 2, 2").toString(),
    readConfig("columns", "2, 2, 2, 2, 2, 2, 2, 2, 2, 2").toString()
  ),
  gap: {
  ...
  minSpaces: Algorithm.createMinSpaces(
    readConfig(
      "minSpaceNames",
      "texstudio, inkscape, krita, gimp, designer, creator, kdenlive, kdevelop, chromium, kate, spotify"
    ).toString(),
    readConfig("minSpaceValues", "1, 1, 1, 1, 1, 1, 1, 2, 2, 2, 2").toString()
  ),
  ignored: {
    names: Algorithm.trimSplitString(
      "ksmserver, krunner, lattedock, Plasma, plasma, plasma-desktop, plasmashell, plugin-container, ".concat(
        readConfig("ignoredNames", "wine, yakuake").toString()
      )
    ),
    captions: Algorithm.trimSplitString(
      readConfig(
        "ignoredCaptions",
        "Trace Bitmap (Shift+Alt+B), Document Properties (Shift+Ctrl+D)"
      ).toString()
    ),
  },
};
...
function Column() {
  ...
  this.addClient = function (
    client // the size is the total size of a virtual desktop this column can occupy
  ) {
    this.clients.push(client);
    if (this.clients.length !== 0) this.dividers.push(0); // do not add a new divider when the first client is added to the column
    return 0;
  };

  this.removeClient = function (clientIndex) {
    if (clientIndex < 0 || clientIndex >= this.clients.length) return -1;
    this.clients.splice(clientIndex, 1);
    if (clientIndex === 0) {
      if (this.dividers.length > 0) this.dividers.splice(0, 1);
    } else {
  ...
  this.swapClient = function (amount, clientIndex) {
    ...
    if (i < 0 || i >= this.clients.length) return -1;
  ...
  this.render = function (
    ...
    var clientHeight =
      (areaHeight -
        ...
        (this.clients.length - nminimized + 1) * gap) /
      (this.clients.length - nminimized);
    ...
    for (var i = 0; i < this.clients.length; i++) {
      ...
      if (
        ...
        (i < this.clients.length - 1 && this.clients[i + 1].minimized)
      )
        divider = 0;
...
function Screen() {
  ...
  this.addColumn = function (column, maxColumns, index) {
    if (this.columns.length >= maxColumns) return -1;
    ...
    for (var i = 0; i < this.columns.length; i++) {
      if (this.columns[i].minSpace() > 1 / (this.columns.length + 1)) return -1;
    }

    // do not add a new divider when the first column is added to the desktop
    if (index === undefined) {
      this.columns.push(column);
      if (this.columns.length > 1) this.dividers.push(0);
    } else {
      this.columns.splice(index, 0, column);
      if (this.columns.length > 1) this.dividers.splice(index, 0, 0);
    }
    return 0;
  };

  this.removeColumn = function (columnIndex) {
    if (columnIndex < 0 || columnIndex >= this.columns.length) return -1;
  ...
  this.smallestColumn = function () {
    ...
    for (var i = 0; i < this.columns.length; i++) {
      ...
      if (minSpace < space) {
        ...
        if (
          this.columns[i].clients.length < this.columns[index].clients.length
        ) {
          index = i;
          space = minSpace;
        }
  ...
  this.addClient = function (client, maxRows, maxColumns) {
    ...
    if (
      index !== -1 &&
      this.columns[index].minSpace() + client.minSpace <=
        1 / this.columns.length &&
      this.columns[index].clients.length < maxRows
    ) {
      if (
        this.columns.length >= maxColumns ||
        this.columns.length > this.columns[index].clients.length
      )
        return this.columns[index].addClient(client);
    }

    if (
      client.minSpace > 1 / (this.columns.length + 1) ||
      this.addColumn(new Column(), maxColumns) === -1
    )
      return -1;
  ...
  this.swapColumn = function (amount, columnIndex) {
    ...
    if (i < 0 || i >= this.columns.length) return -1;
  ...
  this.swapClient = function (amount, clientIndex, columnIndex) {
    if (columnIndex < 0 || columnIndex >= this.columns.length) return -1;
    var column = this.columns[columnIndex];
    if (clientIndex < 0 || clientIndex >= column.clients.length) return -1;
  ...
  this.moveClient = function (
    ...
    while (
      ...
      (this.columns[i].minSpace() + client.minSpace > 1 / this.columns.length ||
        this.columns[i].clients.length >= maxRows)
    )
      i += amount;
    ...
    if (i < 0 || i >= this.columns.length) {
      ...
      c.addClient(client);
      if (i < 0) {
        if (this.addColumn(c, maxColumns, 0) === -1) return -1;
        return this.removeClient(clientIndex, columnIndex + 1);
      } else {
        if (this.addColumn(c, maxColumns) === -1) return -1;
  ...
  this.render = function (screenIndex, desktopIndex, activityName) {
    ...
    var columnWidth =
      (area.width -
        ...
        (this.columns.length - nminimized + 1) * gap) /
      (this.columns.length - nminimized); // width per column
    ...
    for (var i = 0; i < this.columns.length; i++) {
      ...
      if (
        ...
        (i < this.columns.length - 1 &&
          this.columns[i + 1].nminimized() ===
            this.columns[i + 1].clients.length)
      )
        divider = 0;
      else divider = this.dividers[i];
...
function Desktop() {
  ...
  this.addScreen = function (screen) {
    if (this.screens.length >= workspace.numScreens) return -1;
  ...
  this.addClient = function (client) {
    ...
    do {
      while (i >= this.screens.length) {
        if (this.addScreen(new Screen()) !== 0) return -1;
      }
      if (
        this.screens[i].addClient(
          client,
          Parameters.grids[i].row,
          Parameters.grids[i].column
        ) === 0
      )
        return 0;
      if (++i >= workspace.numScreens) i = 0;
    } while (i !== client.screen);
    return -1;
  };

  this.removeClient = function (clientIndex, columnIndex, screenIndex) {
    if (screenIndex < 0 || screenIndex >= this.screens.length) return -1;
  ...
  this.moveClient = function (
    ...
    var client = this.screens[screenIndex].columns[columnIndex].clients[
      clientIndex
    ];
    while (targetIndex >= this.screens.length) {
      if (this.addScreen(new Screen()) === -1) return -1;
    }

    if (this.screens[targetIndex].addClient(client) === -1) return -1;
...
function Activity() {
  ...
  this.addDesktop = function (desktop) {
    if (this.desktops.length >= workspace.desktops) return -1;
  ...
  this.addClient = function (client) {
    ...
    do {
      while (i >= this.desktops.length) {
        if (this.addDesktop(new Desktop()) !== 0) return -1;
      }
  ...
  this.removeClient = function (
    ...
    if (desktopIndex < 0 || desktopIndex >= this.desktops.length) return -1;
    if (
      this.desktops[desktopIndex].removeClient(
        clientIndex,
        columnIndex,
        screenIndex
      ) !== 0
  ...
  this.moveClient = function (
    ...
    if (
      desktopIndex < 0 ||
      desktopIndex >= this.desktops.length ||
      targetIndex < 0 ||
      targetIndex === desktopIndex
    )
      return -1;

    var client = this.desktops[desktopIndex].screens[screenIndex].columns[
      columnIndex
    ].clients[clientIndex];
    while (targetIndex >= this.desktops.length) {
      if (this.addDesktop(new Desktop()) === -1) return -1;
    }

    if (this.desktops[targetIndex].addClient(client) === -1) return -1;
    return this.desktops[desktopIndex].removeClient(
      clientIndex,
      columnIndex,
      screenIndex
    );
...
function Layout() {
  ...
  this.removeClient = function (
    ...
    if (
      this.activities[activityName].removeClient(
        clientIndex,
        columnIndex,
        screenIndex,
        desktopIndex
      ...
      return -1;
    if (
      this.activities[activityName].desktops.length === 0 ||
      workspace.activities.indexOf(activityName) === -1
    )
  ...
  this.moveClient = function (client, activityName) {
    if (
      client.activityName === activityName ||
      !this.activities.hasOwnProperty(client.activityName)
    )
      return -1;
...
var Client = {
  ...
  managed: function (client) {
    return (
      floatingClients.hasOwnProperty(client.windowId) ||
      tiledClients.hasOwnProperty(client.windowId)
    );
  },
  ignored: function (client) {
    if (
      ...
      client.dialog ||
      Parameters.ignored.captions.indexOf(client.caption.toString()) !== -1
    )
      return true;
    for (var i = 0; i < Parameters.ignored.names.length; i++) {
      if (
        client.resourceClass.toString().indexOf(Parameters.ignored.names[i]) !==
          -1 ||
        client.resourceName.toString().indexOf(Parameters.ignored.names[i]) !==
          -1
      )
        return true;
    }
  ...
  addMinSpace: function (client) {
    ...
    for (var name in Parameters.minSpaces) {
      if (
        client.resourceClass.toString().indexOf(name) !== -1 ||
        client.resourceName.toString().indexOf(name) !== -1
      ) {
  ...
  resized: function (client, screen) {
    ...
    var nclients =
      screen.columns[client.columnIndex].clients.length -
      screen.columns[client.columnIndex].nminimized();
    ...
    if (diff.width !== 0) {
      if (diff.x === 0)
        screen.changeDivider(
          "right",
          diff.width / columnWidth,
          client.columnIndex
        );
      else
        screen.changeDivider(
    ...
    if (diff.height !== 0) {
      if (diff.y === 0)
        screen.columns[client.columnIndex].changeDivider(
          "bottom",
          diff.height / clientHeight,
          client.clientIndex
        );
      else
        screen.columns[client.columnIndex].changeDivider(
  ...
  moved: function (client, columns) {
    ...
    var remainder =
      client.geometry.x +
      0.5 * client.geometry.width -
      Parameters.margin.left -
      area.x;
    for (; i < columns.length && remainder > 0; i++) {
      if (columns[i].nminimized() === columns[i].clients.length) continue;
    ...
    remainder =
      client.geometry.y +
      0.5 * client.geometry.height -
      Parameters.margin.top -
      area.y;
    for (; j < columns[i].clients.length && remainder > 0; j++) {
      if (columns[i].clients[j].minimized) continue;
      remainder -= columns[i].clients[j].geometry.height + gap;
    }
    if (j-- === 0) return -1;

    if (
      columns[client.columnIndex].minSpace() -
        client.minSpace +
        columns[i].clients[j].minSpace >
      1 / columns.length
    )
      // check if target fit in current
      return 0;
    if (
      columns[i].minSpace() - columns[i].clients[j].minSpace + client.minSpace >
      1 / columns.length
    )
      // check if current fit in target
      return 0;

    columns[client.columnIndex].clients[client.clientIndex] =
      columns[i].clients[j];
...
Client.addSignals = function (client) {
  client.clientFinishUserMovedResized.connect(function (client) {
    ...
    var screen =
      layout.activities[client.activityName].desktops[client.desktopIndex]
        .screens[client.screenIndex];
    if (
      Client.resized(client, screen) === 0 ||
      Client.moved(client, screen.columns) === 0
    )
  ...
  client.desktopChanged.connect(
    (function () {
      ...
      return function () {
        if (
          c.desktop === c.desktopIndex + 1 ||
          !tiledClients.hasOwnProperty(c.windowId)
        )
          return -1;
        ...
        while (
          ...
          if (targetIndex >= workspace.desktops) targetIndex = 0;
          else if (targetIndex < 0) targetIndex = workspace.desktops - 1;
  ...
  client.screenChanged.connect(
    (function () {
      ...
      return function () {
        if (
          c.screen === c.screenIndex ||
          !tiledClients.hasOwnProperty(c.windowId)
        )
          return -1;
        var desktop =
          layout.activities[c.activityName].desktops[c.desktopIndex];
        ...
        while (
          ...
          targetIndex += direction;
          if (targetIndex >= workspace.numScreens) targetIndex = 0;
          else if (targetIndex < 0) targetIndex = workspace.numScreens - 1;
          if (targetIndex === start) break;
        }
  ...
  client.activitiesChanged.connect(function (client) {
    if (!tiledClients.hasOwnProperty(client.windowId)) return -1;
    client = tiledClients[client.windowId];
    if (client.activities.length !== 1) return Client.float(client);
...
workspace.clientMinimized.connect(function (client) {
  if (!tiledClients.hasOwnProperty(client.windowId)) return -1;
  client = tiledClients[client.windowId];
  return layout.activities[client.activityName].desktops[
    client.desktopIndex
  ].screens[client.screenIndex].render(
    client.screenIndex,
    client.desktopIndex,
    client.activityName
  );
...
[
  ...
  GlobalShortcut(
    "Swap " + entry.direction,
    "Meta+Ctrl+" + entry.direction,
    (function () {
      ...
      return function () {
        ...
        if (
          screen.columns[client.columnIndex].swapClient(
            amount,
            client.clientIndex
          ) !== 0
        )
    ...
    (function () {
      ...
      return function () {
        ...
        if (
          ...
          screen.swapClient(amount, client.clientIndex, client.columnIndex) !==
            0
        )
          return -1;
...
GlobalShortcut("Move Next Desktop/Screen", "Meta+End", function () {
  ...
  if (client.desktop < workspace.desktops)
    // indexing of desktops starts at 1 by Kwin
    client.desktop++;
  else client.desktop = 1;
  workspace.currentDesktop = client.desktop;
});
...
GlobalShortcut("Move Previous Desktop/Screen", "Meta+Home", function () {
  ...
  if (client.desktop > 1)
    // indexing of desktops starts at 1 by Kwin
    client.desktop--;
  else client.desktop = workspace.desktops;
  workspace.currentDesktop = client.desktop;
});
  ...
  GlobalShortcut(
    ...
    (function () {
      ...
      return function () {
        ...
        screen.columns[client.columnIndex].changeDivider(
          "both",
          amount * Parameters.divider.step,
          client.clientIndex
        );
        screen.changeDivider(
          "both",
...
GlobalShortcut("Minimize Others/Unminimize Desktop", "Meta+M", function () {
  ...
  var screen =
    layout.activities[workspace.currentActivity].desktops[
...
GlobalShortcut("Maximize Next", "Meta+N", function () {
  if (!layout.activities.hasOwnProperty(workspace.currentActivity)) return -1;
  var screen =
    layout.activities[workspace.currentActivity].desktops[
      workspace.currentDesktop - 1
  ...
  if (nclients - nminimized <= 1) {
    ...
    // get current and next window's position
    var getPositionInfo = function () {
      for (i = 0; i < screen.columns.length; i++) {
        ...
        for (j = 0; j < column.clients.length; j++) {
          ...
          if (column.clients[j].windowId === client.windowId) {
            // Find next window
            if (column.clients[j + 1]) {
              return { curCol: i, curCli: j, nxtCol: i, nxtCli: j + 1 };
            } else {
              // Next column
              if (screen.columns[i + 1]) {
                if (screen.columns[i + 1].clients[0]) {
                  return { curCol: i, curCli: j, nxtCol: i + 1, nxtCli: 0 };
                } else return null;
              }
              // Cycle back to first column
              else if (i !== 0 && screen.columns[0]) {
                if (screen.columns[0].clients[0]) {
                  return { curCol: i, curCli: j, nxtCol: 0, nxtCli: 0 };
                } else return null;
              }
              // 1 column, cycle back to first client of this column
              else if (column.clients[0]) {
                return { curCol: i, curCli: j, nxtCol: i, nxtCli: 0 };
              }
              // Nothing available
              else return null;
            }
    ...
    var data = getPositionInfo();
    if (data) {
      // Minimize current window
      screen.columns[data.curCol].clients[data.curCli].minimized = true;

      // Unminimize target window
      screen.columns[data.nxtCol].clients[data.nxtCli].minimized = false;
      workspace.activeClient = screen.columns[data.nxtCol].clients[data.nxtCli];
...
GlobalShortcut("Maximize Previous", "Meta+P", function () {
  if (!layout.activities.hasOwnProperty(workspace.currentActivity)) return -1;
  var screen =
    layout.activities[workspace.currentActivity].desktops[
      workspace.currentDesktop - 1
    ].screens[workspace.activeScreen];
  var i, j, column, colLen;
  var nclients = 0,
    nminimized = 0;
  for (i = 0; i < screen.columns.length; i++) {
    nclients += screen.columns[i].clients.length;
  ...
  if (nclients - nminimized <= 1) {
    ...
    // get current and previous window's position
    var getPositionInfo = function () {
      var maxCol = screen.columns.length;
      for (i = 0; i < maxCol; i++) {
        column = screen.columns[i];
        colLen = column.clients.length;
        ...
        for (j = 0; j < column.clients.length; j++) {
          ...
          if (column.clients[j].windowId === client.windowId) {
            ...
            if (column.clients[j - 1]) {
              ...
              // Previous column
              if (screen.columns[i - 1]) {
                var len = screen.columns[i - 1].clients.length;
                if (screen.columns[i - 1].clients[len - 1]) {
                  return {
                    curCol: i,
                    curCli: j,
                    nxtCol: i - 1,
              ...
              // Cycle back to last column
              else if (i === 0 && maxCol > 1 && screen.columns[maxCol - 1]) {
                var len = screen.columns[maxCol - 1].clients.length;
                if (screen.columns[maxCol - 1].clients[len - 1]) {
                  return {
                    curCol: i,
                    curCli: j,
                    nxtCol: maxCol - 1,
              ...
              // 1 column, cycle back to last client of this column
              else if (column.clients[colLen - 1]) {
                return { curCol: i, curCli: j, nxtCol: i, nxtCli: colLen - 1 };
              }
              // Nothing available
              else return null;
            }
...
GlobalShortcut("Tile/Float", "Meta+T", function () {
  var client = workspace.activeClient;
  if (!Client.valid(client) || Client.ignored(client)) return -1;
  else if (floatingClients.hasOwnProperty(client.windowId))
...
function InitClients() {
  var clients = workspace.clientList();
  for (var i = 0; i < clients.length; i++) Client.init(clients[i]);
}
InitClients();
